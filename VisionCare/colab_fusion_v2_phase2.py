# ╔══════════════════════════════════════════════════════════════════════╗
# ║  VISIONCARE 2.0 — PHASE 2 EXTENDED FUSION TRAINING                 ║
# ║  (colab_fusion_v2_phase2.py — copy of V2 with Tier 3 targets)      ║
# ║                                                                      ║
# ║  CHANGES vs colab_fusion_v2.py:                                     ║
# ║   • OUT_DIR     → VisionCare_V2_Phase2/  (separate from V2)        ║
# ║   • TARGETS     → configurable: HF + Mortality + Tier 3 diseases   ║
# ║   • Checkpoint  → fusion_v2_phase2_best.pth                        ║
# ║   • Figures     → ROC, PR, Confusion Matrix, Gate distribution      ║
# ║                   + full previous-presentation comparison           ║
# ║                                                                      ║
# ║  SYMILE-MIMIC data shapes (from official docs):                     ║
# ║   • CXR  : (n, 3, 320, 320) — already normalised                  ║
# ║   • ECG  : (n, 1, 5000, 12) — normalized to [-1, 1]               ║
# ║   • Labs : (n, 50) percentiles + (n, 50) missingness               ║
# ║                                                                      ║
# ║  To add Tier 3 diseases: set ENABLE_TIER3 = True below             ║
# ║  (requires diagnoses_icd.csv with appropriate ICD codes)            ║
# ╚══════════════════════════════════════════════════════════════════════╝
...
# If not found → only HF + Mortality trained (safe fallback)
# ══════════════════════════════════════════════════════════════════════
...
print(f"\n🔍 diagnoses_icd.csv {'✅ FOUND' if ENABLE_TIER3 else '❌ NOT FOUND'} → "
      f"Tier 3 diseases {'ENABLED' if ENABLE_TIER3 else 'DISABLED (HF+Mortality only)'}")

TIER3_TARGETS = [
    # disease name              ICD-10 prefix(es)
    ('myocardial_infarction',  ['I21', 'I22']),       # STEMI/NSTEMI
    ('arrhythmia',             ['I47', 'I48', 'I49']), # VT/AF/flutter
    ('sepsis',                 ['A40', 'A41']),         # bacterial sepsis
    ('pulmonary_embolism',     ['I26']),                # PE
    ('acute_kidney_injury',    ['N17']),                # AKI
    ('icu_admission',          ['Z99', 'J96']),         # ICU-level care
]
...
class Config:
    ...
    NPY_DIR    = f"{BASE}/data_npy"
    # ★ CHANGED: separate output folder from V2
    OUT_DIR    = f"{BASE}/VisionCare_V2_Phase2"
    CKPT_DIR   = f"{BASE}/MultiLabel_Results/checkpoints"   # Phase-1 checkpoints
    CKPT_V2    = f"{OUT_DIR}/checkpoints"                   # Phase-2 outputs go here
    MIMIC_DIR  = f"{BASE}/mimic-iv-csv"

    # ── Phase-1 checkpoint filenames (unchanged) ───────────────────────
    ...
    # ── Targets: always HF + Mortality; Tier 3 appended if enabled ─────
    ...
    # ★ CHANGED: checkpoint name distinguishes from V2
    CKPT_NAME  = "fusion_v2_phase2_best.pth"
    REPORT_NAME = "fusion_v2_phase2_report.json"
    ...
    def setup(cls):
        ...
        print(f"\n{'='*65}")
        print("⚙️   VISIONCARE 2.0 — PHASE 2 EXTENDED CONFIG")
        print(f"{'='*65}")
        ...
        print(f"  Checkpoint      : {cls.CKPT_NAME}")
        print(f"{'='*65}\n")
...
def extract_labels(csv_dir, mimic_dir, split, master_df):
    ...
    # ── 3. TIER 3: all ICD diseases from same diagnoses_icd.csv ────────
    if ENABLE_TIER3:
        if os.path.exists(diag_path):
            ...
            for disease_name, icd_prefixes in TIER3_TARGETS:
                ...
                print(f"  ✅ {disease_name:<28}: {n_pos:4d} pos "
                      f"({100*n_pos/len(df):.1f}%)  [ICD: {icd_prefixes}]")
        else:
            print("  ⚠️  diagnoses_icd.csv missing — Tier 3 zero-filled")
...
class FusionTrainerV2:
    ...
    def _save_checkpoint(self, epoch, metrics):
        ckpt_path = f"{self.cfg.CKPT_V2}/{self.cfg.CKPT_NAME}"
        torch.save({
            'epoch':            epoch,
...
# Save JSON — to Phase2-specific output dir
report = {
    'model':        'VisionCare 2.0 Phase 2 — Cross-Attention Fusion',
    'script':       'colab_fusion_v2_phase2.py',
    'targets':      Config.TARGETS,
    'tier3_enabled': ENABLE_TIER3,
    'trained_at':   datetime.now().isoformat(),
    'best_epoch':   trainer.best_epoch,
    'phase1': {'vision': v_auc, 'signal': s_auc, 'clinical': c_auc,
               'fusion_v1': 0.7702, 'fusion_v2': 0.8105},   # V2 baseline for comparison
    ...
    'table_8_1': {   # exact values from thesis Table 8.1 (V2 baseline)
        'heart_failure': {'auc':0.8189,'f1':0.5888,'prec':0.584,'rec':0.589,'spec':0.821},
        'mortality':     {'auc':0.8022,'f1':0.3115,'prec':0.452,'rec':0.422,'spec':0.930},
    }
}
rpath = f"{Config.OUT_DIR}/{Config.REPORT_NAME}"
...
COLS = {'vision':'#3498db', 'signal':'#e74c3c', 'clinical':'#9b59b6',
        'fusion_v2':'#27ae60', 'p1_fusion':'#bdc3c7',
        'mort':'#e74c3c', 'hf':'#3498db'}

# ── Collect validation probs/labels for curve figures ────────────────
print("\n📊 Collecting validation outputs for figures...")
model.eval()
all_lbl_fig, all_prob_fig, all_gates_fig = [], [], []
with torch.no_grad():
    for cxr, ecg, labs, lbl in val_loader:
        cxr  = cxr.to(Config.DEVICE)
        ecg  = ecg.to(Config.DEVICE)
        labs = labs.to(Config.DEVICE)
        logits, gates = model(cxr, ecg, labs)
        all_lbl_fig.append(lbl.numpy())
        all_prob_fig.append(torch.sigmoid(logits).cpu().numpy())
        all_gates_fig.append(gates.cpu().numpy())
val_labels = np.concatenate(all_lbl_fig)   # (N, num_targets)
val_probs  = np.concatenate(all_prob_fig)
val_gates  = np.concatenate(all_gates_fig) # (N, 3)

ep = range(1, len(history['train_loss']) + 1)  # ★ FIXED typo from original


# ════════════════════════════════════════════════════════════════════
# FIG A — Training History (Loss + AUC + LR)
# ════════════════════════════════════════════════════════════════════
fig, axes = plt.subplots(1, 3, figsize=(16, 4))
axes[0].plot(ep, history['train_loss'], 'b-o', ms=5, lw=2)
axes[0].set_title('Training Loss (BCE)', fontweight='bold')
axes[0].set_xlabel('Epoch'); axes[0].set_ylabel('Loss')
axes[0].grid(True, alpha=0.3)

axes[1].plot(ep, history['val_auc'], 'g-o', ms=5, lw=2, label='Val Macro-AUC')
best_ep = int(np.argmax(history['val_auc']))
axes[1].scatter(best_ep+1, history['val_auc'][best_ep], color='red',
                s=200, zorder=6, marker='*', label=f'Best: {history["val_auc"][best_ep]:.4f}')
...
axes[1].axhline(0.8105, color='purple', ls=':', lw=1.5, label='V2 Baseline: 0.8105')
axes[1].set_title('Validation Macro-AUC', fontweight='bold')
axes[1].set_xlabel('Epoch'); axes[1].legend(fontsize=7); axes[1].grid(True, alpha=0.3)
...
plt.suptitle('VisionCare 2.0 Phase 2 — Training History', fontsize=14, fontweight='bold')
plt.tight_layout()
savefig("p2_training_history.png")


# ════════════════════════════════════════════════════════════════════
# FIG B — Phase Comparison Bar (all phases + V2 baseline)
# ════════════════════════════════════════════════════════════════════
fig, ax = plt.subplots(figsize=(13, 6))
xlabels = ['Vision\n(P1)', 'Signal\n(P1)', 'Clinical\n(P1)',
           'Fusion\nP1', 'Fusion\nV2★', f'Phase2\n({len(Config.TARGETS)} targets)']
values  = [v_auc, s_auc, c_auc, 0.7702, 0.8105, best['macro_auc']]
clrs    = [COLS['vision'], COLS['signal'], COLS['clinical'],
           COLS['p1_fusion'], COLS['fusion_v2'], '#f39c12']
bars = ax.bar(xlabels, values, color=clrs, edgecolor='black', lw=1.5, width=0.6)
...
ax.set_ylim(max(0, min(values)-0.06), min(1.0, max(values)+0.09))
ax.set_title('VisionCare — Full Performance Journey (P1 → P2)', fontsize=14, fontweight='bold')
ax.legend(fontsize=10); ax.grid(True, axis='y', alpha=0.3)
plt.tight_layout()
savefig("p2_phase_comparison.png")


# ════════════════════════════════════════════════════════════════════
# FIG C — ROC Curves (all targets)
# ════════════════════════════════════════════════════════════════════
fig, ax = plt.subplots(figsize=(8, 7))
roc_colors = [COLS['mort'], COLS['hf'], '#9b59b6', '#f39c12', '#1abc9c', '#e67e22']
for i, t in enumerate(Config.TARGETS):
    ...
    ax.plot(fpr, tpr, lw=2.5, color=roc_colors[i % len(roc_colors)],
            label=f"{t.replace('_',' ').title()}  (AUC={auc_val:.4f})")
    ax.fill_between(fpr, tpr, alpha=0.06, color=roc_colors[i % len(roc_colors)])
ax.plot([0,1],[0,1],'k--', alpha=0.4, lw=1.5, label='Random (AUC=0.50)')
ax.set_xlabel('False Positive Rate (1 – Specificity)', fontsize=12)
ax.set_ylabel('True Positive Rate (Sensitivity)', fontsize=12)
ax.set_title('ROC Curves — VisionCare 2.0 Phase 2', fontweight='bold', fontsize=13)
ax.legend(fontsize=9, loc='lower right')
ax.set_xlim(0,1); ax.set_ylim(0,1)
plt.tight_layout()
savefig("p2_roc_curves.png")


# ════════════════════════════════════════════════════════════════════
# FIG D — Precision-Recall Curves
# ════════════════════════════════════════════════════════════════════
fig, ax = plt.subplots(figsize=(8, 7))
for i, t in enumerate(Config.TARGETS):
    if val_labels[:, i].sum() == 0:
        continue
    prec_c, rec_c, _ = precision_recall_curve(val_labels[:, i], val_probs[:, i])
    ap = average_precision_score(val_labels[:, i], val_probs[:, i])
    baseline = val_labels[:, i].mean()
    ax.plot(rec_c, prec_c, lw=2.5, color=roc_colors[i % len(roc_colors)],
            label=f"{t.replace('_',' ').title()}  (AP={ap:.3f})")
    ax.axhline(baseline, color=roc_colors[i % len(roc_colors)],
               ls=':', lw=1, alpha=0.5)
ax.set_xlabel('Recall', fontsize=12)
ax.set_ylabel('Precision', fontsize=12)
ax.set_title('Precision-Recall Curves — VisionCare 2.0 Phase 2', fontweight='bold', fontsize=13)
ax.legend(fontsize=9, loc='upper right')
ax.set_xlim(0,1); ax.set_ylim(0,1.05)
plt.tight_layout()
savefig("p2_pr_curves.png")


# ════════════════════════════════════════════════════════════════════
# FIG E — Confusion Matrices (one per target)
# ════════════════════════════════════════════════════════════════════
from sklearn.metrics import confusion_matrix as sk_cm
ncols = min(len(Config.TARGETS), 3)
nrows = (len(Config.TARGETS) + ncols - 1) // ncols
fig, axes = plt.subplots(nrows, ncols, figsize=(6*ncols, 5*nrows))
axes = np.array(axes).flatten() if len(Config.TARGETS) > 1 else [axes]
cmaps = ['Reds','Blues','Purples','Oranges','Greens','YlOrRd']
for i, t in enumerate(Config.TARGETS):
    preds = (val_probs[:, i] > 0.5).astype(int)
    cm_arr = sk_cm(val_labels[:, i], preds)
    m = best['per_class'][t]
...
for j in range(len(Config.TARGETS), len(axes)):
    axes[j].set_visible(False)
plt.suptitle('Confusion Matrices — VisionCare 2.0 Phase 2', fontsize=13, fontweight='bold')
plt.tight_layout()
savefig("p2_confusion_matrices.png")


# ════════════════════════════════════════════════════════════════════
# FIG F — Dynamic Gate Weights Over Epochs
# ════════════════════════════════════════════════════════════════════
gw_arr = np.array(history['gates'])  # (epochs, 3)
fig, axes = plt.subplots(1, 2, figsize=(14, 5))
cols3 = [COLS['vision'], COLS['signal'], COLS['clinical']]
for i, (c, l) in enumerate(zip(cols3, mods)):
    axes[0].plot(ep, gw_arr[:, i], color=c, lw=2.5, marker='o', ms=4, label=l)
axes[0].set_xlabel('Epoch', fontsize=12); axes[0].set_ylabel('Gate Weight', fontsize=12)
axes[0].set_title('Modality Gate Evolution During Training', fontweight='bold')
axes[0].legend(fontsize=10); axes[0].grid(True, alpha=0.3)
axes[0].set_ylim(0.1, 0.65)

# Right: final avg gate pie
final_gw = gw_arr[-1]
axes[1].pie(final_gw,
            labels=[f'{l}\n{100*g:.1f}%' for l,g in zip(['Vision','ECG','Labs'],final_gw)],
            colors=cols3, autopct='%1.1f%%', startangle=90,
            wedgeprops={'edgecolor':'white','lw':2.5},
            textprops={'fontsize':11,'fontweight':'bold'}, pctdistance=0.75)
axes[1].set_title('Final Epoch Gate Weights', fontweight='bold')
plt.suptitle('VisionCare 2.0 — Dynamic Modality Contributions (XAI)', fontsize=13, fontweight='bold')
plt.tight_layout()
savefig("p2_modality_gates.png")


# ════════════════════════════════════════════════════════════════════
# FIG G — Per-Patient Gate Distribution (XAI)
# ════════════════════════════════════════════════════════════════════
fig, axes = plt.subplots(1, 3, figsize=(15, 4))
gate_names = ['Vision (CXR)', 'Signal (ECG)', 'Clinical (Labs)']
for i, (ax, gname, col) in enumerate(zip(axes, gate_names, cols3)):
    g_vals = val_gates[:, i]
    ax.hist(g_vals, bins=30, color=col, alpha=0.8, edgecolor='white', lw=0.5)
    ax.axvline(g_vals.mean(), color='black', ls='--', lw=2,
               label=f'Mean={g_vals.mean():.3f}')
    ax.axvline(gw[i], color='gold', ls='-', lw=2.5,
               label=f'Train avg={gw[i]:.3f}')
    ax.set_xlabel('Gate Weight (per patient)', fontsize=11)
    ax.set_ylabel('Count', fontsize=11)
    ax.set_title(gname, fontweight='bold')
    ax.legend(fontsize=9)
plt.suptitle(f'Per-Patient Gate Distributions (n={len(val_gates)} validation patients)',
             fontsize=13, fontweight='bold')
plt.tight_layout()
savefig("p2_patient_gates.png")


# ════════════════════════════════════════════════════════════════════
# FIG H — Full Metrics Summary Table (publication-ready)
# ════════════════════════════════════════════════════════════════════
fig, ax = plt.subplots(figsize=(14, 2 + len(Config.TARGETS)*0.7))
ax.axis('off')
cols_hdr = ['Target', 'AUC-ROC', 'F1-Score', 'Precision', 'Recall', 'Specificity', 'Support']
rows = []
for t in Config.TARGETS:
    m = best['per_class'][t]
    n_neg = len(val_labels) - m['support']
    preds_t = (val_probs[:, Config.TARGETS.index(t)] > 0.5).astype(int)
    tn_v = int(((preds_t == 0) & (val_labels[:, Config.TARGETS.index(t)] == 0)).sum())
    spec  = tn_v / max(n_neg, 1)
    rows.append([t.replace('_',' ').title(),
                 f"{m['auc']:.4f}", f"{m['f1']:.4f}",
                 f"{m['precision']:.3f}", f"{m['recall']:.3f}",
                 f"{spec:.3f}", str(m['support'])])
# Macro row
mac_prec = np.mean([best['per_class'][t]['precision'] for t in Config.TARGETS])
mac_rec  = np.mean([best['per_class'][t]['recall']    for t in Config.TARGETS])
rows.append(['Macro Average',
             f"{best['macro_auc']:.4f}", f"{best['macro_f1']:.4f}",
             f"{mac_prec:.3f}", f"{mac_rec:.3f}", '—', '—'])
tbl = ax.table(cellText=rows, colLabels=cols_hdr,
               cellLoc='center', loc='center',
               colWidths=[0.22,0.1,0.1,0.1,0.1,0.12,0.1])
...
plt.title('VisionCare 2.0 Phase 2 — Test Set Performance Summary',
          fontweight='bold', fontsize=13, pad=15)
plt.tight_layout()
savefig("p2_metrics_table.png")


# ════════════════════════════════════════════════════════════════════
# FIG I — Per-disease AUC vs F1 bar comparison
# ════════════════════════════════════════════════════════════════════
if len(Config.TARGETS) > 1:
    fig, axes = plt.subplots(1, 2, figsize=(13, 5))
    bar_colors = [COLS['mort'], COLS['hf'], '#9b59b6', '#f39c12', '#1abc9c', '#e67e22']
    for ax, mk, ml in zip(axes, ['auc', 'f1'], ['AUC-ROC', 'F1-Score']):
        vals = [best['per_class'][t][mk] for t in Config.TARGETS]
        brs  = ax.bar([t.replace('_','\n') for t in Config.TARGETS], vals,
                      color=bar_colors[:len(vals)], edgecolor='black', alpha=0.9, width=0.5)
        for b, v in zip(brs, vals):
            ax.text(b.get_x()+b.get_width()/2, b.get_height()+0.01,
                    f'{v:.3f}', ha='center', fontsize=11, fontweight='bold')
        ax.set_ylim(0, 1.15); ax.set_title(f'{ml} per Target', fontweight='bold', fontsize=12)
        ax.grid(True, axis='y', alpha=0.3)
    plt.suptitle('Per-Disease Performance — VisionCare 2.0 Phase 2', fontsize=13, fontweight='bold')
    plt.tight_layout()
    savefig("p2_per_disease.png")
